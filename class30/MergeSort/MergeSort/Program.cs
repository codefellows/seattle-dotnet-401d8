...
namespace MergeSort
{
	class Program
	{
		...
		{
			// i is the pointer for array b, set it to the beginning
			int i = 0;
			// j is the pointer for array c, set it to the beginning
			int j = 0;
			// k is the pointer for the main array to populate
			int k = 0;
			...
			while (i < b.Length && j < c.Length)
			{
				...
				// if pointer b is less than pointer c value put that value of pointer b into the main array (a)
				if (b[i] <= c[j])
				{
					// 
					a[k] = b[i];
				...
				{
					// if the value of pointer c is less, then put that in the main array
					a[k] = c[j];
